...
const getEdition = async (idEdition: number) =>{
  await axios
      .get(`https://localhost:5001/edition/showInfoEdition?idEdition=${idEdition}`)
      .then((response) =>{
...
<template>
  <div v-if="editionInfo" class="form">
    ...
    <Select :initial-value="workModel[editionInfo.mode]"
        boxName="Work Model"
    ...
    <TextArea :initial-value="editionInfo.description"
        boxName="Description"
        v-bind:minLength="10"
        v-bind:maxLength="500"
        @descriptionText="handleDescription"
    ></TextArea>
    <TextArea :initial-value="editionInfo.curriculum"
        boxName="Curriculum"
