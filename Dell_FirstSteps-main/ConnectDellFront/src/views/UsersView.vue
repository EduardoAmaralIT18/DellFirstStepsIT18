...
const deleteUser = async () => {
  await axios
    .delete(`https://localhost:5001/User/removeUser/${selectedUser.value?.id}`)
    .then(async () => {
      await findAllUsers();
...
const updateUser = async (user: User) => {
  await axios
    .patch(
      `https://localhost:5001/User/changeRole?userId=${user.id}&role=${user.role}`
    )
    .then(async () => {
      await findAllUsers();
...
const findAllUsers = async () => {
  await axios
    .get("https://localhost:5001/User/listUsers")
    .then((response) => {
  ...
  <div>
    ...
    <div class="container">
      <table id="components-table--static" class="dds__table">
        ...
        <tbody class="dds__tbody">
          <tr class="dds__tr" v-for="user in pagedUsers">
            ...
            <td class="dds__td" id="delete-td">
              <button
                v-if="!itsMe(user.id)"
                class="red dds__button dds__button--destructive"
                type="button"
                @click="selectUser(user)"
              >
                <i
                  class="dds__icon dds__icon--user-remove"
                  aria-hidden="true"
                ></i>
              </button>
            </td>
        ...
        <div>
          <div
            class="pagination dds__pagination"
            data-dds="pagination"
            id="516534716"
            role="navigation"
            ...
            <div class="dds__pagination__nav">
              ...
              <div class="dds__pagination__page-range">
                <label
                  ...
                  for="pagination-current-page-516534716"
                  >Page</label
                >
                <div
